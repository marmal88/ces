...
def main():
    ...
    st.write(
        "This is a simple front end to enable users to make predictions on possible."
    )
    st.write(
        "Simply fill in the details of the profile of cluster and a prediction will be created."
    )
    ...
    housing_median_age = col1.number_input(
        "Median age of house in cluster?", value=16, step=1
    )
    ...
    if predict:
        ...
        st.write(f"The predicted Median CES score is {prediction.tolist()[0]:.2f}.")
        st.write(f"This prediction is taken from path {path}")
...
def load_model():
    """
    Load the model and return it for inference.

    Returns:
        sklearn.model: Model as per the config file setting.
    """
