...
class Training_Pipeline:
    ...
    def __init__(self, df_location):
        ...
        # Pre select the features to drop from the dataframe
        self.X = self.df.drop(columns=[ "median_ces", 
                            "longitude", 
                            "latitude", 
                            "total_bedrooms", 
                            ])
        ...
        self.scoring = { 'MAE': make_scorer(mean_absolute_error),
    ...
    def hyperparameter_tuning(self, cv_num:str):
        ...
        self.ct = ColumnTransformer([
                                ...
                                ("standard_scaler", StandardScaler(), ["housing_median_age", "median_income"]),
                                ("ordinal_encoder", OrdinalEncoder(categories=[categories]), ["ocean_proximity"])
                            ], remainder="drop")
        self.ct.fit(X_train, y_train)

        # Hyperparameter tuning for Linear Regression
        lin_reg_pipe = Pipeline(steps=[("column_transfomer", self.ct), ("lin_reg", LinearRegression())])
